...
aceBT_bleGattClientCallbacks_t gatt_client_callback = {
    ...
    //     aceBtCli_bleGattcServiceDiscoveredCallback,
    // .on_ble_gattc_read_characteristics_cb = aceBtCli_bleGattcReadCharsCallback,
    // .on_ble_gattc_write_characteristics_cb =
    //     aceBtCli_bleGattcWriteCharsCallback,
    // .notify_characteristics_cb = aceBtCli_bleGattcNotifyCharsCallback,
    // .on_ble_gattc_write_descriptor_cb = aceBtCli_bleGattcWriteDescCallback,
    // .on_ble_gattc_read_descriptor_cb = aceBtCli_bleGattcReadDescCallback,
    // .on_ble_gattc_get_gatt_db_cb = aceBtCli_bleGattcGetDbCallback,
    // .on_ble_gattc_execute_write_cb = aceBtCli_bleGattcExecuteWriteCallback,
};
...
int main() {
    ...
    // Apparently required for LE pairing
    ace_status_t security_result = aceBT_registerAsSecurityClient(bt_session, &bt_security_callbacks);
    printf("Registered as security client %d\n", security_result);

    ace_status_t ble_status = aceBT_bleRegister(bt_session, &ble_callbacks);
    printf("BLE registration %d\n", ble_status);

    // These two functions are pretty much the same.
    // Both exist in bluetooth_ble_gatt_client_api.h
    // Neither work. Both produce undefined reference.
